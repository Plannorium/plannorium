...
const index = () => {
  ...
  const services = ["Custom Software Development", "Mobile App Development", "UX/UI Design", "AI Integration", "Automation", "Platform & Infrastructure"]
  ...
  return (
    <section className="min-h-screen services-section" ref={container}>
      ...
      <div className="items">
        <motion.div className="top-border" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} transition={{ duration: 2, ease: "easeInOut" }} />
        {services.map((service, index) => (
          <div className="item" key={index}>
            <p>{service}</p>
            <Image src="/images/arrow.png" width={52.34} height={0} alt="icon" />
            <motion.div className="border-line bottom" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} transition={{ duration: 2, ease: "easeInOut" }} />
          </div>
        ))}
      </div>
